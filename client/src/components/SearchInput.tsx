...
const SearchInput = () => {
  ...
  const handleSubmit = useCallback(
    (e: FormEvent<HTMLFormElement>) => {
      ...
      const searchedConversation = conversations.find((see) =>
        see?.fullName.toLowerCase().includes(search.toLowerCase())
      );
  ...
  return (
    <form onSubmit={handleSubmit} className="flex items-center gap-2">
      <input
        ...
        className="input input-bordered rounded-2xl h-9"
      />
      <button className="bg-sky-500 hover:bg-yellow-400 duration-500 ease-in-out rounded-full p-2 text-center">
